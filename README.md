...
`grid-snake` は、グリッド状に繋がる一筆書き状のヘビ状パスを自動生成するジェネラティブアートツール。
直感的なコントロールパネルから行数・列数、角丸率、色テーマ、ヘビの太さ、外郭線太さ、芯の太さなどのパラメータをカスタマイズ可能。
仮想の高さ関係（外郭線 ＞ 芯 ＞ 罫線）に基づく物理的な立体・内部シャドウ（図形内部の陰影・ベベル・ハイライト）や、7種のアーティスティック・エフェクト（フィルムグレイン、CMYK印刷、リソグラフ風印刷、網点ハーフトーン、カラーディザリング、インク染み・滲み、和紙の質感）と組み合わせてリッチなグラフィック表現を創出できます。
...
└── src/
    ...
    │   │   ├── relief3dRenderer.ts - 立体・内部シャドウ (外郭線＞芯＞罫線の仮想高さ・内部陰影)
    ...
    └── components/
        ...
        └── sections/           - 設定カテゴリごとの個別UIコンポーネント
            ...
            ├── RenderingStyleSection.tsx - 描画スタイル & 3D立体 / エフェクト
...
- 立体・内部シャドウ（3Dインナー）: 「外郭線 ＞ 芯 ＞ 罫線」の固定された仮想高さに基づき、図形外部ではなく図形内部に発生するリッチな陰影・ベベル・光沢ハイライトをリアルタイムレンダリング
